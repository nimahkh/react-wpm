...
function Result(props) {
  ...
  function reTest() {
    ...
    dispatch({type: 'startTheGame', start: false})
    dispatch({type: 'changeErrors', stop: ''})

    setTimeout(() => {
      dispatch({type: 'stopTimer', stop: false})
    }, 500)
  }
  ...
  function save() {
    ...
    const data = {
      result: ` wpm: ${WPMResult.wpm} and the percentage is : ${parseFloat(percentage).toFixed(2)} % | Date: ${date} `
    }
  ...
  return (<React.Fragment>
    <div className="result flex">
      ...
      {
        responseId && <div className="YourId">
      ...
      <ul>
        ...
        <li>percentage : {percentage.toFixed(0)}
          %
        </li>
      </ul>
